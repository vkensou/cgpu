...
/* clang-format off */
static CGPU_FORCEINLINE bool FormatUtil_IsDepthStencilFormat(ECGPUFormat const fmt) {
    switch(fmt) {
...
static CGPU_FORCEINLINE bool FormatUtil_IsDepthOnlyFormat(ECGPUFormat const fmt) {
    switch(fmt) {
        case CGPU_FORMAT_D32_SFLOAT:
        case CGPU_FORMAT_D16_UNORM:
            return true;
...
static CGPU_FORCEINLINE uint32_t FormatUtil_BitSizeOfBlock(ECGPUFormat const fmt) {
	switch(fmt) {
...
static CGPU_FORCEINLINE uint32_t FormatUtil_WidthOfBlock(ECGPUFormat const fmt) {
    switch(fmt) {
...
static CGPU_FORCEINLINE uint32_t FormatUtil_HeightOfBlock(ECGPUFormat const fmt) {
	switch(fmt) {
		case CGPU_FORMAT_UNDEFINED: return 1;
		case CGPU_FORMAT_DXBC1_RGB_UNORM: return 4;
		case CGPU_FORMAT_DXBC1_RGB_SRGB: return 4;
