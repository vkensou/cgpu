...
{
    const cgpu_allocator_t* allocator = &Inst->allocator;
...
// Shader Reflection
static const cgpu_resource_type_flag RTLut[] = {
...
static cgpu_texture_dimension_enum DIMLut[SpvDimSubpassData + 1] = {
...
static cgpu_texture_dimension_enum ArrDIMLut[SpvDimSubpassData + 1] = {
...
void VkUtil_InitializeShaderReflection(CGPUDeviceId device, CGPUShaderLibrary_Vulkan* S, const struct CGPUShaderLibraryDescriptor* desc)
{
    ...
    for (uint32_t i = 0; i < entry_count; i++)
    {
        ...
        reflection->stage = (cgpu_shader_stage_flag)entry->shader_stage;
...
{
    const cgpu_allocator_t* allocator = &S->super.device->adapter->instance->allocator;
...
{
    cgpu_adapter_detail_t* adapter_detail = &VkAdapter->adapter_detail;
...
CGPU_FORCEINLINE void VkUtil_CheckFormatSupport(CGPUAdapter_Vulkan* VkAdapter, cgpu_adapter_detail_t* adapter_detail, uint32_t i)
{
...
void VkUtil_EnumFormatSupports(CGPUAdapter_Vulkan* VkAdapter)
{
    ...
    cgpu_adapter_detail_t* adapter_detail = (cgpu_adapter_detail_t*)&VkAdapter->adapter_detail;
...
{
    const cgpu_allocator_t* allocator = &vkInstance->super.allocator;
    uint32_t count = 0;
    vkEnumerateInstanceLayerProperties(&count, NULL);
    if (count != 0)
    {
        vkInstance->pLayerNames = cgpu_calloc(allocator, instance_layers_count, sizeof(const char*));
    ...
    const char* const* instance_extensions, uint32_t instance_extension_count)
{
    ...
    if (count > 0)
    {
        VkInstance->pExtensionProperties = cgpu_calloc(allocator, instance_extension_count, sizeof(VkExtensionProperties));
    ...
    const char* const* device_layers, uint32_t device_layers_count, const cgpu_allocator_t* allocator)
{
    ...
    if (count != 0)
    {
        VkAdapter->pLayerNames = cgpu_calloc(allocator, device_layers_count, sizeof(const char*));
    ...
    const char* const* device_extensions, uint32_t device_extension_count, const cgpu_allocator_t* allocator)
{
    ...
    if (count > 0)
    {
        VkAdapter->pExtensionProperties = cgpu_calloc(allocator, device_extension_count, sizeof(VkExtensionProperties));
