...
CGPUBufferId cgpu_create_buffer_vulkan(CGPUDeviceId device, const struct CGPUBufferDescriptor* desc)
{
    ...
    CGPU_DECLARE_ZERO(VmaAllocationInfo, alloc_info)
    ...
    if ((add_info.usage & VK_BUFFER_USAGE_UNIFORM_TEXEL_BUFFER_BIT) || (add_info.usage & VK_BUFFER_USAGE_STORAGE_TEXEL_BUFFER_BIT))
    {
        ...
        CGPU_DECLARE_ZERO(VkFormatProperties, formatProps)
        ...
        if (add_info.usage & VK_BUFFER_USAGE_UNIFORM_TEXEL_BUFFER_BIT)
        {
            ...
            {
                cgpu_warn(device->adapter->instance, "Failed to create uniform texel buffer view for format %u", (uint32_t)desc->format);
            }
            else
            {
                CHECK_VKRESULT(device->adapter->instance, vkCreateBufferView(D->pVkDevice, &viewInfo, GLOBAL_VkAllocationCallbacks, &B->pVkUniformTexelView));
            }
        }
        ...
        if (add_info.usage & VK_BUFFER_USAGE_STORAGE_TEXEL_BUFFER_BIT)
        {
            ...
            {
                cgpu_warn(device->adapter->instance, "Failed to create storage texel buffer view for format %u", (uint32_t)desc->format);
            }
            else
            {
                CHECK_VKRESULT(device->adapter->instance, vkCreateBufferView(D->pVkDevice, &viewInfo, GLOBAL_VkAllocationCallbacks, &B->pVkStorageTexelView));
            }
        }
    }
    // Set Buffer Name
    ...
    if (isSinglePlane)
    {
        ...
        VkBufferImageCopy copy = {
            ...
            .imageSubresource = {
                ...
                .baseArrayLayer = desc->dst_subresource.base_array_layer,
            ...
            for (uint32_t x = start.x; x < end.x; x++)
            {
                VkBufferImageCopy copy = {
                    ...
                    .imageSubresource = {
                        ...
                        .baseArrayLayer = desc->region.layer,
                        .layerCount = 1,
                    },
                    .imageOffset = {
                        .x = (int32_t)(x * TileWidth),
                        .y = (int32_t)(y * TileHeight),
                        .z = (int32_t)(z * TileDepth),
                    },
                    .imageExtent = {
                        .width = TileWidth,
                        .height = TileHeight,
    ...
    VkImage* outImage, VkDeviceMemory* outMemory)
{
    ...
    {
        cgpu_error(D->super.adapter->instance, "Failed to find memory type for image");
    }
    // import memory
    VkResult importRes = D->mVkDeviceTable.vkCreateImage(D->pVkDevice, pImageCreateInfo, GLOBAL_VkAllocationCallbacks, outImage);
    CHECK_VKRESULT(D->super.adapter->instance, importRes);
    ...
    cgpu_info(D->super.adapter->instance, "Importing external memory %ls allocation of size %llu", win32Name, importAllocation.allocationSize);
    ...
    {
        cgpu_trace(D->super.adapter->instance, "Imported image %p with allocation %p", *outImage, *outMemory);
    }
}
    ...
    VkExportMemoryWin32HandleInfoKHR* pWin32ExportMemoryInfo)
{
    ...
    cgpu_trace(D->super.adapter->instance, "Exporting texture with name %ls size %dx%dx%d", win32Name, desc->width, desc->height, desc->depth);
#else
...
VkSparseImageMemoryRequirements VkUtil_FillTiledTextureInfo(CGPUDevice_Vulkan* D, CGPUTexture_Vulkan* T, const struct CGPUTextureDescriptor* desc, uint32_t* outTypeBits)
{
    ...
    {
        cgpu_error(D->super.adapter->instance, "Requested sparse image size exceeds supported sparse address space size!");
    }
    ...
    CGPU_DECLARE_ZERO(VkSparseImageMemoryRequirements, sparseReq);
    CGPU_DECLARE_ZERO_VLA(VkSparseImageMemoryRequirements, sparseMemoryReqs, sparseMemoryReqsCount);
    if (sparseMemoryReqsCount == 0)
    {
        cgpu_error(D->super.adapter->instance, "No memory requirements for the sparse image!");
    }
    else
    {
    ...
    if (!colorAspectFound)
    {
        ...
        {
            cgpu_error(D->super.adapter->instance, "Only found non-standard sparse block size layout!");
        }
        cgpu_error(D->super.adapter->instance, "Could not find sparse image memory requirements for color aspect bit!");
    }
    ...
    cgpu_assert(sparseImageMemoryReqs.size == VK_SPARSE_PAGE_STANDARD_SIZE * total_tiles_count && "Unexpected sparse image memory requirements size!");
...
CGPUTextureId cgpu_create_texture_vulkan(CGPUDeviceId device, const struct CGPUTextureDescriptor* desc)
{
    ...
    {
        cgpu_error(device->adapter->instance, "Multi-Sampled textures cannot have mip maps");
        cgpu_assert(false);
        return CGPU_NULLPTR;
    }
    ...
    if (pVkImage == VK_NULL_HANDLE)
    {
        ...
        CGPU_DECLARE_ZERO(VkFormatProperties, format_props);
        ...
        if ((desc->flags & CGPU_TCF_ALIASING_RESOURCE) || (desc->flags & CGPU_TCF_TILED_RESOURCE))
        {
            ...
            CHECK_VKRESULT(device->adapter->instance, res);
        }
        else
        {
...
{
    cgpu_assert(subres->mappings && x < subres->X && y < subres->Y && z < subres->Z && "SubresTileMappings::at: Out of Range!"); 
...
{
    cgpu_assert(mip_level < T->super.info->mip_levels && array_index < T->super.info->array_size_minus_one + 1);
...
void cgpu_queue_map_packed_mips_vulkan(CGPUQueueId queue, const struct CGPUTiledTexturePackedMips* regions)
{
    ...
    CGPU_DECLARE_ZERO(VkBindSparseInfo, bindSparseInfo);
...
void cgpu_queue_map_tiled_texture_vulkan(CGPUQueueId queue, const struct CGPUTiledTextureRegions* regions)
{
    ...
    for (uint32_t i = 0; i < RegionCount; i++)
    {
        ...
        uint32_t RegionTileCount = 0;
            ...
            {
                cgpu_assert(Region.mip_level < pTiledInfo->packed_mip_start && 
	...
	CGPU_DECLARE_ZERO(VkSparseImageMemoryBindInfo, imageMemoryBindInfo);
...
void cgpu_free_texture_vulkan(CGPUTextureId texture)
{
    ...
    if (T->pVkImage != VK_NULL_HANDLE)
    {
        ...
        else if (pInfo->owns_image)
        {
            ...
            {
                cgpu_trace(D->super.adapter->instance, "Freeing texture allocation %p \n\t size: %dx%dx%d owns_image: %d imported: %d",
        ...
        {
            cgpu_trace(D->super.adapter->instance, "Freeing texture %p \n\t size: %dx%dx%d owns_image: %d imported: %d",
                    T->pVkImage, pInfo->width, pInfo->height, pInfo->depth, pInfo->owns_image, pInfo->is_imported);
...
CGPUTextureViewId cgpu_create_texture_view_vulkan(CGPUDeviceId device, const struct CGPUTextureViewDescriptor* desc)
{
    ...
    switch (mImageType)
    {
        ...
        case VK_IMAGE_TYPE_3D:
            ...
            {
                cgpu_error(device->adapter->instance, "Cannot support 3D Texture Array in Vulkan");
                cgpu_assert(false);
            }
    ...
    {
        CHECK_VKRESULT(device->adapter->instance, D->mVkDeviceTable.vkCreateImageView(D->pVkDevice, &srvDesc, GLOBAL_VkAllocationCallbacks, &TV->pVkSRVDescriptor));
    }
    // UAV
    if (desc->usages & CGPU_TVU_UAV)
    {
        ...
        CHECK_VKRESULT(device->adapter->instance, D->mVkDeviceTable.vkCreateImageView(D->pVkDevice, &uavDesc, GLOBAL_VkAllocationCallbacks, &TV->pVkUAVDescriptor));
    }
    // RTV & DSV
    if (desc->usages & CGPU_TVU_RTV_DSV)
    {
        CHECK_VKRESULT(device->adapter->instance, D->mVkDeviceTable.vkCreateImageView(D->pVkDevice, &srvDesc, GLOBAL_VkAllocationCallbacks, &TV->pVkRTVDSVDescriptor));
    }
    return &TV->super;
}
...
CGPUSamplerId cgpu_create_sampler_vulkan(CGPUDeviceId device, const struct CGPUSamplerDescriptor* desc)
{
    ...
    CHECK_VKRESULT(device->adapter->instance, D->mVkDeviceTable.vkCreateSampler(D->pVkDevice, &sampler_info, GLOBAL_VkAllocationCallbacks, &S->pVkSampler));
    return &S->super;
}
