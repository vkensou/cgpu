...
function name_converter.enum_name(enum)
    return "cgpu_" .. upperCamelcase_to_underscorecase(enum.name) .. "_enum"
end
...
function name_converter.flag_name(flag)
    return "cgpu_" .. upperCamelcase_to_underscorecase(flag.name) .. "_flag"
end
...
local function gen_enum_cdefine(enum, enum_temp)
    ...
    for index, item in ipairs(enum.enum) do
        ...
        items[#items + 1] = string.format("%s,%s /** (%2d) %s%s */", name, namealign(name, 40), index - 1, comment,
...
local function text_with_comments(items, item, is_classmember)
    local name = item.name
    if item.array then
    ...
    if item.comment then
        if #item.comment > 1 then
            ...
            text = string.format(false and "%s %s/** %s%s */" or "%s %s//!< %s", text, namealign(text, 40),
                item.comment[1], namealign(item.comment[1], 40))
        end
    end
    items[#items + 1] = text
end
